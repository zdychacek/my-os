...
void read_sectors_ATA_PIO(uint32_t *buffer, uint32_t LBA, uint8_t sector_count)
{
  ...
  port_byte_write(0x1F6, 0xE0 | ((LBA >> 24) & 0xF));
  port_byte_write(0x1F2, sector_count);
  port_byte_write(0x1F3, (uint8_t)LBA);
  port_byte_write(0x1F4, (uint8_t)(LBA >> 8));
  port_byte_write(0x1F5, (uint8_t)(LBA >> 16));
  port_byte_write(0x1F7, 0x20); // send the read command
...
void write_sectors_ATA_PIO(uint32_t LBA, uint8_t sector_count, uint32_t *buffer)
{
  ...
  port_byte_write(0x1F4, (uint8_t)(LBA >> 8));
  port_byte_write(0x1F5, (uint8_t)(LBA >> 16));
  port_byte_write(0x1F7, 0x30); // send the write command
  ...
  for (int j = 0; j < sector_count; j++)
  {
    ...
    for (int i = 0; i < 256; i++)
    {
      port_long_write(0x1F0, buffer[i]);
    }
  }
}
...
{
  while (port_byte_read(0x1F7) & STATUS_BSY)
    ;
}
static void ATA_wait_DRQ() //Wait fot drq to be 1
{
  while (!(port_byte_read(0x1F7) & STATUS_RDY))
    ;
}
